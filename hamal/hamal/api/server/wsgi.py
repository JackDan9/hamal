...
from hamal.wsgi import common as wsgi
...
class Request(webob.Request):
    ...
    def best_match_content_type(self):
        ...
        if 'hamal.best_content_type' not in self.environ:
            ...
            self.environ['hamal.best_content_type'] = (content_type or
                                                      'application/json')

        return self.environ['hamal.best_content_type']
...
class JSONDictSerializer(DictSerializer):
    ...
    def default(self, data):
        """Use dump_as_bytes() to ensure that
        the result type is `bytes` on Python2
        and python3.
        """
...
class ResourceExceptionHandler(object):
    ...
    def __exit__(self, ex_type, ex_value, ex_traceback):
        ...
        if isinstance(ex_value, exception.NotAuthorized):
            ...
            LOG.info("Fault thrown: %s", unicode(ex_value))
            raise ex_value
        elif isinstance(ex_value, webob.exc.HTTPException):
            LOG.info("HTTP exception thrown: %s", unicode(ex_value))
...
def _set_request_id_header(req, headers):
    context = req.environ.get('hamal.context')
    if context:
