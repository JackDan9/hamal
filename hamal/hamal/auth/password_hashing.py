...
from hamal.i18n import _, _LC, _LI, _LW, _LE
...
# NOTE(jackdan): Build the list of prefixes. This compresssion builds
    ...
    try:
        if len(password) > max_length:
            if CONF.strict_password_check:
                raise exception.PassowordValidationError(size=max_length)
            else:
...
def hash_user_password(user):
    ...
    return dict(user, password=hash_user_password(password))
